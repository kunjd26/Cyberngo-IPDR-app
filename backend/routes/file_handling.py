...
from src.dynamic_parser import get_processed_file_header
from src.file_status import update_file_status

file_header_bp = Blueprint("file_header", __name__)
processed_file_header_bp = Blueprint("processed_file_header", __name__)
...
# Get header of file
@processed_file_header_bp.route("/api/ipdr-files/processed-header", methods=["GET"])
def get_processed_file_header_endpoint():
    """
    Get Processed File Header Endpoint
    ---
    parameters:
      - name: token
        in: query
    ...
    try:
        ...
        # Parse file through general parser.
        status, result = get_processed_file_header(file_token)
